...
We call this value the \textcolor{red}{\em derivative}\index{derivative} of the function \(y=f(x)\) at the point \(x\) and write
\[
\frac{\dd f}{\dd x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h}
\]
...
\begin{solution}
\begin{gather*}
\frac{\dd f}{\dd x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\to0}\frac{(x+h)^2 - x^2}{h} = \lim\limits_{h\to0}\frac{(x^2+2xh+h^2)-x^2}{h}\\ = \lim\limits_{h\to0}\frac{2xh+h^2}{h} = \lim\limits_{h\to0}(2x+h) = 2x.
\end{gather*}
\end{solution}
...
\begin{solution}
\begin{gather*}
\frac{\dd f}{\dd x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\to0}\frac{(x+h)^3 - x^3}{h} = \lim\limits_{h\to0}\frac{(x^3+3x^2h+3xh^2+h^3)-x^2}{h}\\ = \lim\limits_{h\to0}\frac{3x^2h+3xh^2+h^3}{h} = \lim\limits_{h\to0}(3x^2+3xh+h^2) = 3x^2.
\end{gather*}
\end{solution}
...
Hence
\begin{gather*}
\frac{\dd f}{\dd x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\to0}\frac{-2}{(2x+2h-1)(2x-1)} = \frac{-2}{(2x-1)(2x-1)} = \frac{-2}{(2x-1)^2}.
\end{gather*}
\end{solution}
...
\[
\frac{\dd f}{\dd x} = \frac{\dd y}{\dd x} = f'(x).
\]
...
\[
\frac{\dd y}{\dd x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h} = nx^{n-1}.
\]
Therefore, in general, we can differentiate \(x^n\) for all values of \(n\) using the rule
\[
\frac{\dd y}{\dd x} = \frac{d(x^n)}{\dd x} = nx^{n-1}.
\]
...
\[
y = ax^n \Rightarrow \frac{\dd y}{\dd x} = nax^{n-1}.
\]
...
\begin{solution}
\begin{gather*}
\frac{\dd y}{\dd x} = 3\times4x^3 = 12 x^3.
\end{gather*}
\end{solution}
...
\begin{align*}
\frac{\dd f}{\dd x} &= \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\to0}\frac{ag(x+h)-ag(x)}{h} \\ &= a\lim\limits_{h\to0}\frac{g(x+h)-g(x)}{h} = a\frac{\dd g}{\dd x}.
\end{align*}
...
\begin{solution}
\begin{gather*}
\frac{\dd y}{\dd x} = 4\times5x^4 + 3\times 2x^1 - 6\times1 = 20x^4+6x-6
\end{gather*}
\end{solution}
...
\begin{align*}
\frac{\dd(x^n)}{\dd x} &= nx^{n-1}\text{ for }n\in{\mathbb R}\\
\frac{\dd(ag(x))}{\dd x} &= a\frac{\dd g}{\dd x}\\
\frac{\dd(f(x)+g(x))}{\dd x} &= \frac{\dd f}{\dd x}+\frac{\dd g}{\dd x}.
\end{align*}
\slide
...
Two lines are \textcolor{red}{\em parallel}\index{parallel} if their gradients are identical. For example \(y=2x\), \(y=2x-2\) and \(y=2x+3\) all have gradient of \(4\) and hence are all parallel (Figure \ref{fig:05-parallel-lines}).
...
If two lines are perpendicular we may also find a relationship between their gradients. Consider Figure \ref{fig:06-perpendicular-lines}.
...
Here we have the tangent \(PT\) and the normal \(PN\) to a curve. The \textcolor{red}{\em normal}\index{normal} is the line at \(90^\circ\) to the tangent. We already know that the gradient of the tangent is given by
\[
...
\frac{d^3y}{dx^3} = 6
\]
and is called the \textcolor{red}{\em third derivative of $y$}\index{third derivative of $y$}, and \(d^3y/dx^3\) may be differentiated with respect to \(x\) to give
\[
\frac{d^4y}{dx^4} = 0,
\]
the \textcolor{red}{\em fourth derivative of $y$}\index{fourth derivative of $y$}. Once the result of the successive differentiation operations is zero, no further differential coefficients exist.
\slide
...
\(ds/dt\) is the rate at which \(s\) changes for a change in \(t\) or the rate at which distance changes with time, so \(ds/dt\) is the \textcolor{red}{\em velocity}\index{velocity}, \(v\), of the particle in \(ms^{-1}\).
...
\]
\(dv/dt\) is the rate of change of velocity with time or the \textcolor{red}{\em acceleration}\index{acceleration} of the particle.
...
As \(x\) increases from just before \(B\) to just after, the slope of the graph changes from positive to negative. A point such as \(B\) is known as a \textcolor{red}{\em maximum}\index{maximum} or \textcolor{red}{\em local maximum}\index{local maximum}. The word local is used to indicate that this is not the maximum \(y\) value that the curve can have, but is the maximum value in the region between \(A\) and \(C\).
...
\frac{dy}{dx}=0.
\]
However, there is a further situation where the gradient of the graph may be zero, but where the sign of the slope of the graph does not change. Such a point is called a \textcolor{red}{\em point of inflection}\index{point of inflection}.
...
So far we have learnt to integrate simple functions such as \(x^2 + 2x + 3\) and standard integrals such as \(\sin(x), e^x\) etc. If we want to integrate more complicated functions, more advanced techniques will be needed. One very useful method is \textcolor{red}{\em integration by substitution}\index{integration by substitution}. Consider
\[
...
\]
Equation (4) is the formula for \textcolor{red}{\em integration by parts}\index{integration by parts} and can be used to integrate some types of
product.
...
\]
This sometimes known as the \textcolor{red}{\em (Second) Fundamental Theorem of Calculus}\index{(Second) Fundamental Theorem of Calculus}.
\slide
...
The idea behind \textcolor{red}{\em Simpson's rule}\index{Simpson's rule} is that instead of approximating the curve using a series of trapeziums, we approximate by using a series of quadratic polynomials, see Figure \ref{fig:31-simpsons-rule}.
