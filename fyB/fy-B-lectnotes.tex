...
{\centering \includegraphics[width=0.5\linewidth]{fy-B-lectnotes_files/figure-latex/01-exp-graph-1} 
...
Note that the limit is not the same thing as substituting the value of \(x=3\) into the function \(f(x)\), as \(f(x)\) is not defined at the value \(x=3\). We say that \(f\) is \textcolor{red}{\em discontinuous}\index{discontinuous} at \(x=3\).

A function \(f\) of \(x\) is said to be \textcolor{red}{\em continuous}\index{continuous} at the value \(x=a\) if the value of the limit of \(f(x)\) as \(x\) tends to \(a\) is equal to \(f(a)\). In other words
\[
...
For a straight line we can consider the slope or \textcolor{red}{\em gradient}\index{gradient}. This is a measure of how steep the line is, see Figure \ref{fig:02-gradient-line}.
