...
Informally, a \textcolor{purple}{\em limit}\index{limit} is a value which \(f(x)\) gets closer and closer to, but may never actually reach.
...
A function \(f\) of \(x\) is said to be \textcolor{purple}{\em continuous}\index{continuous} at the value \(x=a\) if:
...
Formally, a function \(f\) of \(x\) is said to be \textcolor{purple}{\em discontinuous}\index{discontinuous} at \(x=a\) if it is not continuous at \(x=a\).
\slide
...
For a straight line we can consider the \textcolor{purple}{\em slope}\index{slope} or \textcolor{purple}{\em gradient}\index{gradient}. This is a measure of how steep the line is, see Figure \ref{fig:02-gradient-line}.
...
The \textcolor{purple}{\em gradient}\index{gradient} is defined to be the number
\[
...
\protect\phantomsection\label{div_head}
Let \(h\to 0\)

\protect\phantomsection\label{div_body}

Press the button and see what happens to the line segment as \(h\to0\).

\end{htmlonly}

With some thought, it should be clear that the line segment tends to a line that just meets the curve at the point \((x,f(x))\), and we call this line the \textcolor{purple}{\em tangent}\index{tangent} to the curve at the point \((x,f(x))\).
...
We call the slope of the tangent line the \textcolor{purple}{\em derivative}\index{derivative} of the function \(y=f(x)\) at the point \(x\) and write
\[
\frac{\mathrm{d} f}{\mathrm{d} x} = \lim\limits_{h\to0}\frac{f(x+h)-f(x)}{h}
\]
and we refer to the process of calculating this limit as \textcolor{purple}{\em differentiation by 1st principles}\index{differentiation by 1st principles}.
...
So that
\[
f(x+h)-f(x) = nx^{n-1}h+\frac{n(n-1)}{2!}x^{n-2}h^2+\ldots + h^n,
\]
and thus
\[
\frac{f(x+h)-f(x)}{h} = nx^{n-1}+\frac{n(n-1)}{2!}x^{n-2}h+\ldots + h^{n-1},
\]
\slide
...
Two lines are \textcolor{purple}{\em parallel}\index{parallel} if their gradients are identical. For example \(y=2x\), \(y=2x-2\) and \(y=2x+3\) all have gradient of \(2\) and hence are all parallel (Figure \ref{fig:05-parallel-lines}).
...
The \textcolor{purple}{\em normal}\index{normal} is the line at \(90^\circ\) to the tangent, passing through the point where the tangent touches the curve. Thus in the figure, the tangent is the line \(PT\) and the normal is the line \(PN\). We already know that the gradient of the tangent is given by
\[
...
y-y_1 = m(x-x_1).
\]
So the \textcolor{purple}{\em equation of the tangent}\index{equation of the tangent} is
\[
y - y_1 = \frac{\mathrm{d} y}{\mathrm{d} x}(x-x_1),
\]
while the \textcolor{purple}{\em equation of the normal}\index{equation of the normal} to the curve is
\[
...
\[
y-14=\frac{-1}{9}(x-2) = \frac{-1}{9}x + \frac{2}{9}\text{ and so }y = -\frac{x}{9}+\frac{128}{9}.
\]
\end{solution}
...
\]
The \textcolor{purple}{\em chain rule}\index{chain rule} says that
\[
...
\]
This is called the \textcolor{purple}{\em product rule}\index{product rule}.
\slide
...
This is the formula for the \textcolor{purple}{\em differentiation of a quotient}\index{differentiation of a quotient}.
...
This is called the \textcolor{purple}{\em second derivative of $y$}\index{second derivative of $y$}.
...
\]
and is called the \textcolor{purple}{\em third derivative of $y$}\index{third derivative of $y$}. Taking the derivative of \(\mathrm{d}^{3}y/\mathrm{d} x^{3}\) yields
\[
\frac{\mathrm{d}^{4}y}{\mathrm{d} x^{4}} = 0,
\]
the \textcolor{purple}{\em fourth derivative of $y$}\index{fourth derivative of $y$}.
...
\(\mathrm{d} s/\mathrm{d} t\) is the rate at which \(s\) changes for a change in \(t\), or the rate at which distance changes with time, so \(\mathrm{d} s/\mathrm{d} t\) is the \textcolor{purple}{\em velocity}\index{velocity}, \(v\), of the particle in \(\text{m}\text{s}^{-1}\).
...
\]
\(\mathrm{d} v/\mathrm{d} t\) is the rate of change of velocity with time or the \textcolor{purple}{\em acceleration}\index{acceleration} of the particle.
...
As \(x\) increases from just before \(B\) to just after, the slope of the graph changes from positive to negative. A point such as \(B\) is called a \textcolor{purple}{\em maximum}\index{maximum} or \textcolor{purple}{\em local maximum}\index{local maximum}. The word ``local'' means that this is not the ``global'' maximum \(y\) value of the whole curve, but is the maximum value in the region ``near \(B\)'', e.g.~between \(A\) and \(C\).

As \(x\) increases from just before \(C\) to just after, the slope of the graph changes from negative to positive. A point such as \(C\) is known as a \textcolor{purple}{\em minimum}\index{minimum} or \textcolor{purple}{\em local minimum}\index{local minimum}.

Points such as \(B\) and \(C\) are also sometimes called \textcolor{purple}{\em critical points}\index{critical points} or \textcolor{purple}{\em stationary points}\index{stationary points} or \textcolor{purple}{\em turning points}\index{turning points}. A function can have more than one maximum or minimum along its range of \(x\) values.
\slide
...
\]
However, the converse is not necessarily true: the gradient may be zero, but the sign of the slope of the graph may not change. Such a point is called a \textcolor{purple}{\em point of inflection}\index{point of inflection}.
...
We get the other part of the criterion (called \textcolor{purple}{\em second derivative criterion}\index{second derivative criterion}):
...
An \textcolor{purple}{\em asymptote}\index{asymptote} of a curve is a line such that the distance between the curve and the line approaches zero as one or both of the \(x\) or \(y\) coordinates tends to infinity.
...
So the line \(x=-1\) is a \textcolor{purple}{\em vertical asymptote}\index{vertical asymptote}.

\slide

There is also a \textcolor{purple}{\em horizontal asymptote}\index{horizontal asymptote}. Notice that as \(x\to+\infty\) then \(y\to 0\) (from above), and as \(x\to-\infty\) then \(y\to0\) (from below). So the line \(y=0\) is a horizontal asymptote.
...
An \textcolor{purple}{\em implicit function}\index{implicit function} is one given by an equation (like the one below), rather than in a direct form \(y=\)(some expression with \(x\)). Here we consider \(y\) to depend on \(x\), and \(x\) to be an ``independent'' variable:
\[
y^2 + x^2 + 2xy + 2x = 0.
\]
...
When a function of more than one variable is differentiated with respect to one variable only, this process is called \textcolor{purple}{\em partial differentiation}\index{partial differentiation}.
...
So far we have learnt to integrate ``sums of powers'' functions such as \(x^2 + 2x + 3\) and standard integrals such as \(\sin(x), e^x\) etc. A useful method for more complicated functions is \textcolor{purple}{\em integration by substitution}\index{integration by substitution}. Consider
\begin{equation}
...
\label{eq:6-8}
\end{equation}
Equation \eqref{eq:6-8} is the formula for \textcolor{purple}{\em integration by parts}\index{integration by parts} and can be used to integrate some types of
product.
...
(If the definite integral would be defined as in most textbooks, this statement would be called the \textcolor{purple}{\em First Fundamental Theorem of Calculus}\index{First Fundamental Theorem of Calculus}.)
...
With the \textcolor{purple}{\em mid-ordinate rule}\index{mid-ordinate rule}, the curve for which we want to know the area is divided into strips of equal width \(s\), see Figure \ref{fig:28-graph-vertical-strips}.
...
For the \textcolor{purple}{\em trapezium rule}\index{trapezium rule}, the curve is again divided into strips of equal width \(s\), but this time, the area of each strip is approximated by a trapezium instead of a rectangle, see Figure \ref{fig:30-trapezium-rule}.
...
For \textcolor{purple}{\em Simpson's rule}\index{Simpson's rule}, we approximate the function in each strip by a quadratic polynomial (i.e.~a piece of a parabola), rather than a line as in the previous two rules --- see Figure \ref{fig:31-simpsons-rule}.
...
\item
  ...
  \end{align*}
\item
  This is known as \textcolor{purple}{\em Simpson's rule}\index{Simpson's rule}.
\end{itemize}
...
The \textcolor{purple}{\em RMS}\index{RMS} (or \textcolor{purple}{\em root mean square}\index{root mean square}) value is obtained by taking \(f(x)\), squaring it, finding the mean value and then taking the square root.
...
\begin{example}
\protect\hypertarget{exm:cone-example}{}\label{exm:cone-example}\leavevmode

The straight line \(y=mx\) between \(x=0\) and \(x=h\) is rotated about the \(x\)-axis thus generating a right circular cone. Calculate its volume in terms of \(h\) and \(R\) where \(R\) is the \(y\) value at \(x=h\).
...
\begin{example}
Consider the area in the plane enclosed between the curve \(y=x\cos(x)\), the \(x\)-axis and the lines \(x=0\) and \(x=\frac{\pi}{3}\). Determine the volume of the solid formed by rotating this area about the \(y\)-axis.
\end{example}
...
\begin{align*}
V &=
2\pi\int_{0}^{\pi/3} xy\,\mathrm{d}x
= 2\pi\int_{0}^{\pi/3} x^2\cos(x)\,\mathrm{d}x\\
&= \left\{\text{by parts: }\begin{array}{ll} u=x^2 &  \mathrm{d}v=\cos(x)\,\mathrm{d}x \\ \mathrm{d}u=2x\ \mathrm{d}x & v=\sin(x)\end{array}\right\}\\
&=2\pi\left[x^2\sin(x)\right]_{0}^{\pi/3} - 2\pi\cdot 2\int_{0}^{\pi/3} x\sin(x)\,\mathrm{d}x\\
&= \left\{\text{by parts: }\begin{array}{ll} u=x &  \mathrm{d}v=\sin(x)\,\mathrm{d}x \\ \mathrm{d}u=\ \mathrm{d}x & v=-\cos(x)\end{array}\right\}\\
&=2\pi\cdot\frac{\pi^2}{9}\cdot\frac{\sqrt{3}}{2}-4\pi\left(\left[-x\cos(x)\right]_{0}^{\pi/3} - \int_{0}^{\pi/3} -\cos(x)\,\mathrm{d}x \right)\\
&=\frac{\sqrt{3}\pi^3}{9}-4\pi\cdot\left(-\frac{\pi}{3}\right)\cdot\frac{1}{2} - 4\pi\left[\sin(x)\right]_{0}^{\pi/3}\\
&=\frac{\sqrt{3}\pi^3}{9}+\frac{2\pi^2}{3}-2\sqrt{3}\pi \approx 1.664 \,\text{units}^3\text{ (3 d.p.)}
\end{align*}
\end{solution}
...
\begin{example}
Consider the cone from Example \ref{exm:cone-example}.
Find the surface area of the resultant cone.
\end{example}
...
\begin{example}
Let \(f(x)=\sqrt{x}\) over the interval \([1,4]\). Find the surface area of the surface generated by revolving the graph of \(f\) around the \(x\)-axis. Round the answer to three decimal places.
\end{example}

\begin{solution}
As \(y=\sqrt{x}\), we have \(\frac{\mathrm{d}y}{\mathrm{d}x} = \frac{1}{2\sqrt{x}}\). Thus
\begin{align*}
S&=2\pi\int_{1}^{4} y\sqrt{1+\left(\frac{\mathrm{d}y}{\mathrm{d}x}\right)^2}\ \mathrm{d}x
= 2\pi\int_{1}^{4} \sqrt{x}\sqrt{1+\frac{1}{4x}}\,\mathrm{d}x\\
&= 2\pi\int_{1}^{4} \sqrt{x+\frac{1}{4}}\,\mathrm{d}x
= 2\pi\int_{1}^{4} \left(x+\frac{1}{4}\right)^{1/2}\,\mathrm{d}x\\
&= 2\pi \left[\frac{2}{3}\cdot\left(x+\frac{1}{4}\right)^{3/2} \right]_{1}^{4}
= \frac{\pi}{6}\left(17\sqrt{17} - 5\sqrt{5}\right)
\approx 30.846 \text{ (3 d.p.)}
\end{align*}
\end{solution}
...
Many structures and mechanical systems act as if their masses were concentrated at a single point called the \textcolor{purple}{\em center of mass}\index{center of mass} or \textcolor{purple}{\em centre of gravity}\index{centre of gravity}. We wish to find out how to locate this point. First let us consider the situation in 1-dimension and then in 2-dimensions.
...
Suppose that the \(x\)-axis is supported by a fulcrum at the origin and we have 2 masses, \(m_1\) and \(m_2\), situated at positions \(x_1\) and \(x_2\) from the origin, see Figure \ref{fig:35-mass-along-a-line}. This is similar to a `see-saw'. Does the system balance, or not? The answer, from experience, depends on both the values of the masses and the positions of the objects. The quantity \(m_1x_1\) is referred to as the \textcolor{purple}{\em moment}\index{moment} of the mass \(m_1\) about the origin. Notice that in our figure, \(m_1x_1<0\) and \(m_2x_2>0\).

The \textcolor{purple}{\em principle of moments}\index{principle of moments} says that the system will balance providing
\[
m_1x_1 + m_2x_2 = 0.
\]
...
The point \(\overline x\) is called the \textcolor{purple}{\em center of mass}\index{center of mass} or \textcolor{purple}{\em center of gravity}\index{center of gravity}.
\slide

In order to generalise to an `infinite number of points', we need to use integration, rather than a finite sum.

Consider a rod on Figure \ref{fig:38-2d-rod}: we assume that its \textcolor{purple}{\em density}\index{density} varies with \(x\), i.e.~it is given by a function \(\rho(x)\).
...
A \textcolor{purple}{\em lamina}\index{lamina} is a solid body in the form of a thin sheet of uniform thickness, with uniform density. As a result, the weight is proportional to the area and the formula for the centre of gravity can have the \(m\) values replaced by \(a\) values where \(a\) represents the area of an element and \(A\) is the total area of the lamina. So
\[
\overline x = \frac{\displaystyle\int xa\ \mathrm{d}x}{A},\;\;\; \overline y = \frac{\displaystyle\int ya\ \mathrm{d}y}{A}
\]
Then \(\overline x\) and \(\overline y\) are the coordinates of the \textcolor{purple}{\em centroid of the area}\index{centroid of the area} or the \textcolor{purple}{\em centroid}\index{centroid}.
...
A \textcolor{purple}{\em scalar}\index{scalar} quantity has only size or magnitude (i.e.~all the information is just a single number). For example: mass, time, speed \(\dots\)

A \textcolor{purple}{\em vector}\index{vector} quantity has magnitude and direction. For example: force, velocity, acceleration, moment, \(\dots\)
...
A \textcolor{purple}{\em position vector}\index{position vector} indicates the distance and direction that we have to travel from the origin to get to a point. We use the term \textcolor{purple}{\em displacement vector}\index{displacement vector} to refer to the same concept, but relative to any other point than origin.
...
We have already seen that a vector may be \textcolor{purple}{\em multiplied by a scalar}\index{multiplied by a scalar}. If the scalar is positive this changes the magnitude but not the direction of the vector. If the scalar is negative the vector is turned to the opposite direction, and the magnitude is changed.

\end{notslides}

\begin{slidesonly}

We've seen that a vector may be \textcolor{purple}{\em multiplied by a scalar}\index{multiplied by a scalar}.
...
A \textcolor{purple}{\em unit vector}\index{unit vector} is a vector with magnitude of 1.
...
When written in this form \(\underline r\) is said to be resolved into its \textcolor{purple}{\em component vectors}\index{component vectors}.
...
The \textcolor{purple}{\em direction cosines}\index{direction cosines} of a vector are the cosines of the angles that that vector makes with each of
...
If \(\underline a\) and \(\underline b\) are given in terms of component vectors as \(\underline a = a_1\underline i + a_2\underline j + a_3\underline k\) and \(\underline b = b_1\underline i + b_2\underline j + b_3\underline k\) then the \textcolor{purple}{\em dot product}\index{dot product} \(\underline{a}\cdot \underline{b}\) is defined to be
\[
...
If we have three vectors \(\underline a, \underline b\) and \(\underline c\) then the \textcolor{purple}{\em scalar triple product}\index{scalar triple product} of those vectors is defined to be \(\underline a \cdot(\underline b\times\underline c)\).
...
If we have three vectors \(\underline a, \underline b\) and \(\underline c\) then the \textcolor{purple}{\em vector triple product}\index{vector triple product} of those vectors is defined as
\[
